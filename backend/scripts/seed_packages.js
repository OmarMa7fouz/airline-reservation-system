...
const run = () => {
    console.log("Cleaning and reseeding packages...");
    db.serialize(() => {
        // Clear existing packages
        db.run("DELETE FROM travel_packages", (err) => {
            if (err) {
            ...
            // Reset autoincrement sequence for travel_packages
            db.run("DELETE FROM sqlite_sequence WHERE name='travel_packages'", (err) => {
                 if(err) console.error("Could not reset sequence (ignorable)", err);
            });
            ...
            newPackages.forEach(p => {
                stmt.run([
                    ...
                    if(completed === newPackages.length) {
                        console.log("✅ Successfully inserted " + completed + " packages with images.");
                    }
                });
            });
            stmt.finalize();
